...
- 1920x1080: weisser Grund; das freigestellte Seebild (Nutzer, main_assets/thumb/lake.png - Silhouette mit 2 Schwimmern, Himmel transparent) bedeckt den unteren Teil (Silhouette kommt auf Weiss heraus); darueber gross + zentriert "See" in Helvetica-Klon (Liberation Sans Bold). "See" links + schwarzer Pfeil (Bezier + Kopf) zeigt auf den See; das Seebild fuellt die VOLLE Breite unten und ist vertikal etwas gestaucht (Baumlinie tiefer). Unten rechts eine echte, lesbare Handschrift-Notiz "Wie entsteht ein See?" in pastell-orange (Font: Kalam, SIL OFL, fonts/Hand.ttf). -> scripts/make_thumb.py
