...
## Hauptteil Chunk 15 (DIE BEDINGUNG - die universelle Formel)

- Synthese nach allen Seemachern: jeder See laeuft auf dieselbe Bedingung hinaus. Vollstaendig generierte editoriale Gleichung auf REINEM WEISS (schliesst an c14 an): ① EIN BECKEN (das Wasser haelt) + ② GENUG WASSER (das hineinlaeuft) = EIN SEE.
- Zentrales Element: ein Swiss-Diagramm-Becken-Querschnitt (voller Breite unten, Mulde in der Mitte), das sich synchron zum VO mit Wasser fuellt (blaue Fuellung steigt bei "genug Wasser das hineinlaeuft", Zulauf-Pfeile), "= EIN SEE" stempelt gelb; danach Kipp-Punkt "Fehlt nur eines von beidem -" -> Wasser laeuft ab, Becken leer, '„TROCKENE SENKE"' rot-orange in der leeren Mulde + feine Trockenrisse. Ghost-Wort "WASSER" dahinter, gestaffelte Anton-Formel rechts. -> scripts/gen_c15.py::basin_frames, scripts/build_c15.py
- Keine externen Assets (rein generiert). Typo in die Frames gebacken, keine Laufschrift.
